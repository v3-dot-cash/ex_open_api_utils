defmodule PhoenixEctoOpenApiDemoWeb.NotificationJSON do
  ...
  (`"object_type" => "email"` / `"sms"` / `"webhook"`) into the channel map
  at render time, which is what makes the wire format line up with the
  `oneOf + discriminator` spec the `NotificationResponse` submodule
  declares.
  """
